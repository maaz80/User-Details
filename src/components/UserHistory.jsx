...
const UserHistory = ({ history ,onSelectUser }) => {
    return (
        <div className='h-[30%] w-[47%] bg-white border rounded-lg shadow fixed bottom-3 '>
            <div className='bg-amber-300 absolute top-0 w-[100%] py-1 md:py-2 text-white rounded-b-md poppins-semibold text-lg md:text-xl text-center'>
                History
            </div>
                ...
                {history.length === 0 ? (
                    ...
                    history.map((user, index) => (
                        <div key={index} className='flex items-center space-x-0 md:space-x-2 border rounded-md py-1 px-0.5 md:px-2 mx-[-10px] md:mx-0' onClick={() => onSelectUser(user)}>
                            <img src={user.avatar} alt={user.name} className="w-8 md:w-16 h-8 md:h-16 rounded-full mr-1 md:mr-4" />
