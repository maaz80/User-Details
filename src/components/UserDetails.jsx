...
const UserDetails = ({ user }) => {
  ...
  return (
    <div className={`p-4  border rounded-lg shadow fixed w-[47%] h-[65%] flex flex-col justify-center items-center bg-white `}>
      {user ? (
        <>
        <div className='bg-amber-300 absolute top-0 w-[100%] py-1 md:py-2 text-white rounded-b-md poppins-semibold text-md md:text-xl text-center'>{user.profile.firstName} {user.profile.lastName}</div>
          <img src={user.avatar} alt={user.name} className="w-24 md:w-48 h-24 md:h-48 rounded-full mx-auto mb-4 drop-shadow-lg " />
